...
public class RupayTTUMServiceImpl extends JdbcDaoSupport implements RupayTTUMService {
	...
	public HashMap<String, Object> checkTTUMProcessed(UnMatchedTTUMBean beanObj) {
		...
		try {
			...
			if (beanObj.getTypeOfTTUM().equalsIgnoreCase("LP")) {
				...
				fetch_condition = " WHERE TRAN_DATE = '" + beanObj.getLocalDate() + "'";// AND FILEDATE = (SELECT
																						// TO_DATE(MAX(FILEDATE),'DD/MM/YY')
																						// FROM "+sett_table+")";
				...
				if (beanObj.getStSubCategory().equalsIgnoreCase("ISSUER")) {
					...
					fetch_condition = " WHERE TRAN_DATE = TO_DATE('" + beanObj.getLocalDate() + "','DD/MM/YYYY') ";// AND
																													// FILEDATE
																													// =
																													// (SELECT
																													// MAX(FILEDATE)
			...
			/*
			 * if(beanObj.getFileDate() == null) { logger.info("Filedate is null ");
			 * fileDate =
			 * getJdbcTemplate().queryForObject("SELECT MAX(FILEDATE) FROM "+sett_table, new
			 * Object[] {},String.class); }
			 */
	...
	public HashMap<String, Object> checkReconProcessed(UnMatchedTTUMBean beanObj) {
		...
		try {
			...
			int compareCount = getJdbcTemplate().queryForObject(checkCompareCount,
	...
	public String createTTUMFile(UnMatchedTTUMBean beanObj) {
		...
		try {
			...
			String getData = "select RPAD(ACCOUNT_NUMBER,14,' ') AS ACCOUNT_NUMBER,CURRENCY_CODE,PART_TRAN_TYPE,"
					...
					+ ",to_char(filedate,'DD-MM-YYYY') AS FILEDATE" + " from " + tableName + " WHERE FILEDATE = ?";

			logger.info("Query for getting ttum data is ");
			logger.info(getData);
			List<TTUMBean> ttumBeanObjLst = getJdbcTemplate().query(getData, new Object[] { beanObj.getFileDate() },
			...
			BufferedWriter out = new BufferedWriter(
					new FileWriter(beanObj.getStPath() + File.separator + fileName, true));
			int startLine = 0;
			...
			for (TTUMBean beanObjData : ttumBeanObjLst) {
				...
				lineData.append(
						beanObjData.getAcc_number() + "  " + "INR0391" + "    " + beanObjData.getPart_tran_type());
				lineData.append(beanObjData.getTrans_amount() + beanObjData.getTrans_particular() + "         "
						+ beanObjData.getRemarks());
				lineData.append("                                                                          ");
	...
	public List<Object> getTTUMData(UnMatchedTTUMBean beanObj) {
		...
		try {
			 ...
			 * + ",to_char(TO_DATE(FILEDATE,'DD/MON/YYYY'),'DD/MM/YYYY') AS FILEDATE FROM "
			 * +ttum_tableName +" WHERE FILEDATE = TO_DATE(?,'DD/MON/YYYY')";
			 */
			...
			if (ttum_format != null && ttum_format.equalsIgnoreCase("OLD")) {
				...
				getData = "SELECT ACCOUNT_NUMBER AS ACCOUNT_NUMBER,PART_TRAN_TYPE," + "TRANSACTION_AMOUNT,"
						+ "TRANSACTION_PARTICULAR ,NVL(REFERENCE_NUMBER,' ') AS REMARKS"
						+ ",TO_DATE(FILEDATE,'DD/MM/YY') AS FILEDATE FROM " + ttum_tableName + fetch_condition;
	...
	public List<Object> getVisaTTUMData(UnMatchedTTUMBean beanObj) {
		...
		try {
			...
			if (beanObj.getTypeOfTTUM().equalsIgnoreCase("LP")) {
				...
				ttum_tableName = "TTUM_VISA_ISS_VISA";
				sett_table = "SETTLEMENT_VISA_" + beanObj.getStSubCategory().substring(0, 3) + "_VISA";
				dom_fetch_condition = " WHERE TRAN_DATE = '" + beanObj.getLocalDate() + "'" + // AND FILEDATE = (SELECT
																								// TO_DATE(MAX(FILEDATE),'DD/MM/YY')
																								// FROM "+sett_table+")
																								// "+
				...
				int_fetch_condition = " WHERE TRAN_DATE = '" + beanObj.getLocalDate() + "'"// AND FILEDATE = (SELECT
																							// TO_DATE(MAX(FILEDATE),'DD/MM/YY')
																							// FROM "+sett_table+")"
						+ " and tran_type = 'INTERNATIONAL'";
			} else if (beanObj.getTypeOfTTUM().equalsIgnoreCase("UNRECON2")) {
			...
			if (ttum_format != null && ttum_format.equalsIgnoreCase("OLD")) {
				...
				getdomData = "SELECT ACCOUNT_NUMBER AS ACCOUNT_NUMBER,PART_TRAN_TYPE," + "TRANSACTION_AMOUNT,"
						+ "TRANSACTION_PARTICULAR ,NVL(REFERENCE_NUMBER,' ') AS REMARKS"
						+ ",TO_DATE(FILEDATE,'DD/MM/YY') AS FILEDATE FROM " + ttum_tableName + dom_fetch_condition;
				// +" WHERE FILEDATE = '"+beanObj.getFileDate()+"'
				// ";//TO_DATE(?,'DD/MON/YYYY')";

				getintData = "SELECT ACCOUNT_NUMBER AS ACCOUNT_NUMBER,PART_TRAN_TYPE," + "TRANSACTION_AMOUNT,"
						+ "TRANSACTION_PARTICULAR ,NVL(REFERENCE_NUMBER,' ') AS REMARKS"
						+ ",TO_DATE(FILEDATE,'DD/MM/YY') AS FILEDATE FROM " + ttum_tableName + int_fetch_condition;
			}
			logger.info("Getdata query is " + getdomData);
			logger.info("Getdata query is " + getintData);

			List<Object> DailyDatadom = getJdbcTemplate().query(getdomData, new Object[] {},
					new ResultSetExtractor<List<Object>>() {
						public List<Object> extractData(ResultSet rs) throws SQLException {
							List<Object> beanList = new ArrayList<Object>();
			...
			List<Object> DailyDataInt = getJdbcTemplate().query(getintData, new Object[] {},
					new ResultSetExtractor<List<Object>>() {
						public List<Object> extractData(ResultSet rs) throws SQLException {
							List<Object> beanList = new ArrayList<Object>();
	...
	public List<Object> getRupayTTUMData(UnMatchedTTUMBean beanObj) {
		...
		try {
			...
			if (beanObj.getTypeOfTTUM().equalsIgnoreCase("LP")) {
				...
				sett_table = "SETTLEMENT_RUPAY_" + beanObj.getStSubCategory().substring(0, 3) + "_RUPAY";
				rupay_fetch_condition = " WHERE TRAN_DATE = '" + beanObj.getLocalDate() + "'" + // AND FILEDATE =
																								// (SELECT
																								// TO_DATE(MAX(FILEDATE),'DD/MM/YY')
																								// FROM "+sett_table+")
																								// "+
						"  and tran_type = 'RUPAY'";
				ncmc_fetch_condition = " WHERE TRAN_DATE = '" + beanObj.getLocalDate() + "'" + // AND FILEDATE = (SELECT
																								// TO_DATE(MAX(FILEDATE),'DD/MM/YY')
																								// FROM "+sett_table+")
																								// "+
						"  and tran_type = 'NCMC'";
			...
			if (ttum_format != null && ttum_format.equalsIgnoreCase("OLD")) {
				...
				getdomData = "SELECT ACCOUNT_NUMBER AS ACCOUNT_NUMBER,PART_TRAN_TYPE," + "TRANSACTION_AMOUNT,"
						+ "TRANSACTION_PARTICULAR ,NVL(REFERENCE_NUMBER,' ') AS REMARKS"
						+ ",TO_DATE(FILEDATE,'DD/MM/YY') AS FILEDATE FROM " + ttum_tableName + rupay_fetch_condition;
				// +" WHERE FILEDATE = '"+beanObj.getFileDate()+"'
				// ";//TO_DATE(?,'DD/MON/YYYY')";

				getintData = "SELECT ACCOUNT_NUMBER AS ACCOUNT_NUMBER,PART_TRAN_TYPE," + "TRANSACTION_AMOUNT,"
						+ "TRANSACTION_PARTICULAR ,NVL(REFERENCE_NUMBER,' ') AS REMARKS"
						+ ",TO_DATE(FILEDATE,'DD/MM/YY') AS FILEDATE FROM " + ttum_tableName + ncmc_fetch_condition;
			}
			logger.info("Getdata query is " + getdomData);
			logger.info("Getdata query is " + getintData);

			List<Object> DailyDatarupay = getJdbcTemplate().query(getdomData, new Object[] {},
					new ResultSetExtractor<List<Object>>() {
						public List<Object> extractData(ResultSet rs) throws SQLException {
							List<Object> beanList = new ArrayList<Object>();
			...
			List<Object> DailyDatancmc = getJdbcTemplate().query(getintData, new Object[] {},
					new ResultSetExtractor<List<Object>>() {
						public List<Object> extractData(ResultSet rs) throws SQLException {
							List<Object> beanList = new ArrayList<Object>();
	...
	public List<Object> getNIHTTUMData(UnMatchedTTUMBean beanObj) {
		String ttum_tableName = null;
		List<Object> data = new ArrayList<Object>();
		String fetch_condition = " WHERE TRAN_DATE = '" + beanObj.getLocalDate() + "'";// AND FILEDATE = (SELECT
																						// MAX(FILEDATE) FROM
																						// SETTLEMENT_NFS_ACQ_NFS)";
		String sett_table = "";
		try {
			String getData1 = null;
			String microAtmData = "SELECT ACCOUNT_NUMBER AS ACCOUNT_NUMBER,PART_TRAN_TYPE," + "TRANSACTION_AMOUNT,"
					+ "TRANSACTION_PARTICULAR ,NVL(REFERENCE_NUMBER,' ') AS REMARKS"
					+ ",TO_DATE(FILEDATE,'DD/MM/YY') AS FILEDATE FROM TTUM_NFS_ACQ_nfs " + fetch_condition
					+ " AND ttum_type = 'MICROATM'";
			...
			List<Object> DailyData1 = getJdbcTemplate().query(microAtmData, new Object[] {},
					new ResultSetExtractor<List<Object>>() {
						public List<Object> extractData(ResultSet rs) throws SQLException {
							List<Object> beanList = new ArrayList<Object>();
			...
			List<Object> DailyData2 = getJdbcTemplate().query(getData1, new Object[] {},
					new ResultSetExtractor<List<Object>>() {
						public List<Object> extractData(ResultSet rs) throws SQLException {
							List<Object> beanList = new ArrayList<Object>();
	...
	public HashMap<String, Object> checkTranReconDate(UnMatchedTTUMBean beanObj) {
		...
		try {
			...
			String tableName = "SETTLEMENT_" + beanObj.getCategory() + "_" + beanObj.getStSubCategory().substring(0, 3)
					+ "_";
	...
	public HashMap<String, Object> checkInternationalTTUMProcessed(UnMatchedTTUMBean beanObj) {
		HashMap<String, Object> output = new HashMap<String, Object>();
		try {
			String checkTTUM = "select count(*) from ttum_rupay_int_rupay where filedate = to_date('"
					+ beanObj.getFileDate() + "','dd/mm/yyyy')";

			if (beanObj.getTypeOfTTUM().equalsIgnoreCase("SURCHARGE")) {
				checkTTUM = "select count(*) from ttum_rupay_int_cbs where tran_Date = to_date('"
	...
	public List<Object> getInternationalTTUMData(UnMatchedTTUMBean beanObj) {
		...
		try {
			...
			if (beanObj.getTypeOfTTUM().equalsIgnoreCase("MEMBERFUND")) {
				...
				fetch_condition = " WHERE FILEDATE = '" + beanObj.getFileDate() + "'";// AND FILEDATE = (SELECT
																						// TO_DATE(MAX(FILEDATE),'DD/MM/YY')
																						// FROM "+sett_table+")";
			} else if (beanObj.getTypeOfTTUM().equalsIgnoreCase("SURCHARGE")) {
	...
	public void generateInternationalTTUMFile(String stPath, String FileName, List<Object> TTUMData) {
		...
		try {
			...
			for (int i = 0; i < TTUMData.size(); i++) {
				...
				startLine++;
				lineData = new StringBuffer();
				lineData.append(table_Data.get("ACCOUNT_NUMBER") + "  " + "INR" + table_Data.get("SOL") + "    "
	...
	public HashMap<String, Object> checkNIHRecords(UnMatchedTTUMBean beanObj) {
		...
		try {
			if (beanObj.getTypeOfTTUM().equalsIgnoreCase("UNRECON")) {
				...
				getunreconCount = getJdbcTemplate().queryForObject(checkNIHData,
						new Object[] { beanObj.getLocalDate() }, Integer.class);
			} else {
				checkNIHData = "select count(*) from settlement_nfs_acq_NFS where filedate = (select max(filedate) from settlement_nfs_iss_cbs) "
	...
	public List<Object> getNIHReport(UnMatchedTTUMBean beanObj) {
		...
		try {
			...
			List<Object> DailyData = getJdbcTemplate().query(getInterchange1, new Object[] { beanObj.getLocalDate() },
					new ResultSetExtractor<List<Object>>() {
						public List<Object> extractData(ResultSet rs) throws SQLException {
							List<Object> beanList = new ArrayList<Object>();
	...
	public ArrayList<String> getDailyColumnList(String tableName) {
		...
		String query = "SELECT column_name FROM   all_tab_cols WHERE  table_name = '" + tableName.toUpperCase()
				+ "' and column_name not like '%$%' and column_name not in('FILEDATE','CREATEDDATE','CREATEDBY','CYCLE','UPDATEDDATE','UPDATEDBY')";
	...
	public HashMap<String, Object> checkCardToCardTTUMProcessed(UnMatchedTTUMBean beanObj) {
		HashMap<String, Object> output = new HashMap<String, Object>();
		try {
			String checkTTUM = "select count(*) from ttum_cardtocard_iss_cbs where tran_Date = to_date('"
					+ beanObj.getLocalDate() + "','dd/mm/yyyy')";
	...
	public List<RecordCount> downloadRawdataSummary() {
		String query = "  SELECT COUNT as count , FILE_NAME as file_name , FILEDATE as filedate, NETWORK as network FROM(\r\n"
				...
				+ "FILE_NAME , FILEDATE )   ";
		System.out.println("Raw Data Summary Query: " + query);

		List<RecordCount> query2 = jdbcTemplate.query(query, new RecordCountRowMapper());
	...
	private static class RecordCountRowMapper implements RowMapper<RecordCount> {
		public RecordCount mapRow(ResultSet rs, int rowNum) throws SQLException {
			RecordCount u = new RecordCount();
			u.setCount(rs.getString(1));
			u.setFile_name(rs.getString(2));
			u.setFiledate(rs.getString(3));
			u.setNetwork(rs.getString(4));
			return u;
		}
	...
	@Override
	public String deleteFiles(String data) {
