...
public HashMap<String, Object> validateAdjustmentFileUpload(NFSSettlementBean beanObj)
{
	...
	try
	{
		...
		int filecount = getJdbcTemplate().queryForObject("select count(1) from main_settlement_file_upload where fileid = ? and filedate = to_date(?,'dd/mm/yyyy')", new Object[] {file_id,beanObj.getDatepicker()},
						Integer.class);
	...
	try
	{
		...
		if(result)
		{
			...
			String insertData = "insert into main_settlement_file_upload(fileid, filedate, uploadby, uploaddate, category, upload_flag, file_subcategory,cycle,settlement_flag,interchange_flag,file_count) " + 
					"VALUES('"+file_id+"',to_date('"+beanObj.getDatepicker()+"','dd/mm/yyyy'),'"+beanObj.getCreatedBy()+"',sysdate,'"+beanObj.getCategory()+"','Y','-',"
							+ "'1','N','N','1')";
...
public Boolean AdjustmentRollback(NFSSettlementBean beanObj)
{
	try
	{
			String query = "delete from nfs_adjustment_rawdata where filedate = to_date('"+
