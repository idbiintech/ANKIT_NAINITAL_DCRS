	...
	logger.info("***** ReadNfsRawData.uploadIssuerData Start ****");
		...
		try {
				...
				try {
					...
					String updateQuery ="UPDATE NFS_NFS_iss_RAWDATA SET FPAN = ibkl_encrypt_decrypt.ibkl_set_encrypt_val(FPAN) "
							+ "WHERE CYCLE = '"+cycle+"' AND FILEDATE = TO_DATE('"+setupBean.getFileDate()+"','dd/mm/yyyy')";
	...
	try {
		...
		boolean readdata = false;
			...
			try {
				...
				/*** ENCRYPTING PAN IN RAWDATA ****/
				logger.info("Updation Starting ");
				String updateQuery ="UPDATE NFS_NFS_ACQ_RAWDATA SET FPAN = ibkl_encrypt_decrypt.ibkl_set_encrypt_val(FPAN) "
						+ "WHERE CYCLE = '"+cycle+"' AND FILEDATE = TO_DATE('"+setupBean.getFileDate()+"','DD/MM/YYYY')";
