...
public class ReadRupay {
	...
	public HashMap<String, Object> readData(CompareSetupBean setupBean, Connection con, MultipartFile file,
	...
	public HashMap<String, Object> uploadRupayDomesticData(CompareSetupBean setupBean, Connection con,
...
//		}
		...
		String InsertQuery = "insert into RUPAY_RUPAY_RAWDATA_NAINITAL(DMS_SMS, APPROVAL, CARD_NUMBER, RRN, TRAN_DATE, TRAN_TIME, AMOUNT, TRAN_CODE, FILEDATE, CREATED_BY, CATEGORY,"
				+ "UNIQUE_FILE_NAME) " + "VALUES(?, ?, ?, ?, ?, ?, ?,?, to_date(?,'dd/MM/yyyy'),?,?,?)";
						...
						for (int i = 0; i < elements.size(); i++) {
							...
							if (data.length > 2) {
								...
								if (data[0].equals("AMOUNT")) {
									String amount1 = stLine.substring((start_pos - 1), (Integer.parseInt(data[2])))
											.trim();
									...
									String cardNumber = stLine.substring((start_pos - 1), (Integer.parseInt(data[2])))
											.trim();
...
//					ps.setString(68, utilBean.getnCrdAcpNm());
					...
					/*
					 * String encryptQuery =
					 * "select ibkl_encrypt_decrypt.ibkl_set_encrypt_val('"+pan+"') enc from dual";
					 * PreparedStatement encryp_ps = con.prepareStatement(encryptQuery); ResultSet
					 * rs = encryp_ps.executeQuery(); String encryptedPan = pan; while(rs.next()) {
					 * encryptedPan = rs.getString("enc"); } encryp_ps.close(); rs.close();
					 * logger.info("Encryoted pan is "+encryptedPan); ps.setString(71,
					 * encryptedPan);
					 */
					ps.setString(74, pan);
			...
			try {
				...
				String updateQuery = "update rupay_rupay_rawdata set pan = ibkl_encrypt_decrypt.ibkl_set_encrypt_val(pan) "
						+ "where filedate = tO_DATE('" + setupBean.getFileDate()
						+ "','dd/mm/yyyy') AND SUBSTR(unique_file_name,3,1) ='" + fileName.substring(2, 3) + "' "
...
//				+ "MCC_Code,Merchant_Name," //added MCC_Code,Merchant_Name by int6261 date 03/04/2018
		...
		try {
			...
			// fis = new
			// FileInputStream("\\\\10.143.11.50\\led\\DCRS\\RUPAY_INTERNATIONAL\\"+
			// filename);
...
//					ps.setString(68, utilBean.getnCrdAcpNm());
					...
					logger.info("Before encrypting pan");
					String encryptQuery = "select ibkl_encrypt_decrypt.ibkl_set_encrypt_val('" + pan
							+ "') enc from dual";
				 ...
				 * case "Txn" :
				 * 
				 * break; case"Hdr": hdr="hdr"; break; case"/Hdr": hdr=""; break;
				 * 
				 * //set headers values which are not common case "nDtTmFlGen" : matcher =
				 * TAG_REGEX.matcher(thisLine); matcher.find();
				 * headerUtil.setnDtTmFlGen(matcher.group(1)); break; case "nMemInstCd" :
				 * matcher = TAG_REGEX.matcher(thisLine); matcher.find();
				 * headerUtil.setnMemInstCd(matcher.group(1)); break; case "nUnFlNm" : matcher =
				 * TAG_REGEX.matcher(thisLine); matcher.find();
				 * headerUtil.setnUnFlNm(matcher.group(1)); break; case "nProdCd" : matcher =
				 * TAG_REGEX.matcher(thisLine); matcher.find();
				 * headerUtil.setnProdCd(matcher.group(1)); break; case "nSetBIN" : matcher =
				 * TAG_REGEX.matcher(thisLine); matcher.find();
				 * headerUtil.setnSetBIN(matcher.group(1)); break; case "nFlCatg" : matcher =
				 * TAG_REGEX.matcher(thisLine); matcher.find();
				 * headerUtil.setnFlCatg(matcher.group(1)); break; case "nVerNum" : matcher =
				 * TAG_REGEX.matcher(thisLine); matcher.find();
				 * headerUtil.setnVerNum(matcher.group(1)); break;
				 * 
				 * //set Transaction values case "nAcqInstCd": matcher =
				 * TAG_REGEX.matcher(thisLine); matcher.find();
				 * utilBean.setnAcqInstCd(matcher.group(1)); break;
				 * 
				 * case "nAmtSet": matcher = TAG_REGEX.matcher(thisLine); matcher.find(); double
				 * amtSet = Integer.parseInt(matcher.group(1)); amtSet = amtSet/100;
				 * utilBean.setnAmtSet(String.valueOf(amtSet));
				 * 
				 * break; case "nAmtTxn": matcher = TAG_REGEX.matcher(thisLine); matcher.find();
				 * double amtTxn = Double.parseDouble(matcher.group(1)); amtTxn = amtTxn/100;
				 * 
				 * 
				 * utilBean.setnAmtTxn(String.valueOf(amtTxn));
				 * 
				 * break; case "nApprvlCd": matcher = TAG_REGEX.matcher(thisLine);
				 * matcher.find(); utilBean.setnApprvlCd(matcher.group(1));
				 * 
				 * break; case "nARD": matcher = TAG_REGEX.matcher(thisLine); matcher.find();
				 * utilBean.setnARD(matcher.group(1));
				 * 
				 * break; case "nCcyCdSet": matcher = TAG_REGEX.matcher(thisLine);
				 * matcher.find();
				 * 
				 * utilBean.setnCcyCdSet(matcher.group(1));
				 * 
				 * break; case "nCcyCdTxn": matcher = TAG_REGEX.matcher(thisLine);
				 * matcher.find(); utilBean.setnCcyCdTxn(matcher.group(1));
				 * 
				 * break; case "nConvRtSet": matcher = TAG_REGEX.matcher(thisLine);
				 * matcher.find(); utilBean.setnConvRtSet(matcher.group(1));
				 * 
				 * break; case "nCrdAcpAddAdrs": matcher = TAG_REGEX.matcher(thisLine);
				 * matcher.find(); utilBean.setnCrdAcpAddAdrs(matcher.group(1));
				 * 
				 * break; case "nCrdAcptTrmId": matcher = TAG_REGEX.matcher(thisLine);
				 * matcher.find(); utilBean.setnCrdAcptTrmId(matcher.group(1));
				 * 
				 * break; case "nCrdAcpZipCd": matcher = TAG_REGEX.matcher(thisLine);
				 * matcher.find(); utilBean.setnCrdAcpZipCd(matcher.group(1));
				 * 
				 * break; //common field for transaction and header case "nDtSet": matcher =
				 * TAG_REGEX.matcher(thisLine); matcher.find(); switch(hdr) {
				 * 
				 * case "hdr":
				 * 
				 * headerUtil.setnDtSet(matcher.group(1)); break; default :
				 * utilBean.setnDtSet(matcher.group(1)); break; }
				 * 
				 * break; case "nDtTmLcTxn": matcher = TAG_REGEX.matcher(thisLine);
				 * matcher.find(); utilBean.setnDtTmLcTxn(matcher.group(1));
				 * 
				 * break; //common field for transaction and header case "nFunCd": matcher =
				 * TAG_REGEX.matcher(thisLine); matcher.find(); switch(hdr) {
				 * 
				 * case "hdr": headerUtil.setnFunCd(matcher.group(1)); break; case "Trl":
				 * trl_nFunCd= matcher.group(1); logger.info(trl_nFunCd);
				 * 
				 * break;
				 * 
				 * 
				 * default : utilBean.setnFunCd(matcher.group(1)); break; }
				 * 
				 * break; case "nLtPrsntInd": matcher = TAG_REGEX.matcher(thisLine);
				 * matcher.find(); utilBean.setnLtPrsntInd(matcher.group(1));
				 * 
				 * break;
				 * 
				 * //common field for transaction and header case "nMTI": matcher =
				 * TAG_REGEX.matcher(thisLine); matcher.find(); switch(hdr) {
				 * 
				 * case "hdr": headerUtil.setnMTI(matcher.group(1)); break; default :
				 * utilBean.setnMTI(matcher.group(1));
				 * 
				 * break; }
				 * 
				 * break; case "nPAN": matcher = TAG_REGEX.matcher(thisLine); matcher.find();
				 * utilBean.setnPAN(matcher.group(1));
				 * 
				 * break; //common field for transaction and header case "nRecNum": matcher =
				 * TAG_REGEX.matcher(thisLine); matcher.find(); switch(hdr) {
				 * 
				 * case "hdr": headerUtil.setnRecNum(matcher.group(1)); break; case "Trl":
				 * headerUtil.setTrl_nRecNum(matcher.group(1)); trl_nRecNum=matcher.group(1);
				 * logger.info(trl_nRecNum); break; default :
				 * utilBean.setnRecNum(matcher.group(1)); break; }
				 * 
				 * break; case "nRGCSRcvdDt": matcher = TAG_REGEX.matcher(thisLine);
				 * matcher.find(); utilBean.setnRGCSRcvdDt(matcher.group(1));
				 * 
				 * 
				 * break; case "nSetDCInd": matcher = TAG_REGEX.matcher(thisLine);
				 * matcher.find(); utilBean.setnSetDCInd(matcher.group(1));
				 * 
				 * break; case "nTxnDesInstCd": matcher = TAG_REGEX.matcher(thisLine);
				 * matcher.find(); utilBean.setnTxnDesInstCd(matcher.group(1));
				 * 
				 * break; case "nTxnOrgInstCd": matcher = TAG_REGEX.matcher(thisLine);
				 * matcher.find(); utilBean.setnTxnOrgInstCd(matcher.group(1));
				 * 
				 * break; case "nUID": matcher = TAG_REGEX.matcher(thisLine); matcher.find();
				 * utilBean.setnUID(matcher.group(1));
				 * 
				 * break; case "nFeeDCInd": matcher = TAG_REGEX.matcher(thisLine);
				 * matcher.find(); switch(feesize){
				 * 
				 * case 1 : utilBean.setnFeeDCInd1(matcher.group(1)); break; case 2 :
				 * logger.info("setnFeeDCInd2"); utilBean.setnFeeDCInd2(matcher.group(1));
				 * break; case 3 : logger.info("setnFeeDCInd3");
				 * utilBean.setnFeeDCInd3(matcher.group(1)); break; case 4 :
				 * logger.info("setnFeeDCInd4"); utilBean.setnFeeDCInd4(matcher.group(1));
				 * break; case 5 : logger.info("setnFeeDCInd5");
				 * utilBean.setnFeeDCInd5(matcher.group(1)); break; default: break; } break;
				 * case "nFeeAmt": matcher = TAG_REGEX.matcher(thisLine); matcher.find();
				 * switch(feesize){
				 * 
				 * case 1 : utilBean.setnFeeAmt1(matcher.group(1)); break; case 2 :
				 * logger.info("setnFeeAmt2"); utilBean.setnFeeAmt2(matcher.group(1)); break;
				 * case 3 : logger.info("setnFeeAmt3"); utilBean.setnFeeAmt3(matcher.group(1));
				 * break; case 4 : logger.info("setnFeeAmt4");
				 * utilBean.setnFeeAmt4(matcher.group(1)); break; case 5 :
				 * logger.info("setnFeeAmt5"); utilBean.setFeeAmt5(matcher.group(1)); break;
				 * default: break; } break; case "nFeeCcy": matcher =
				 * TAG_REGEX.matcher(thisLine); matcher.find(); switch(feesize){
				 * 
				 * case 1 :
				 * 
				 * utilBean.setnFeeCcy1(matcher.group(1)); break; case 2 :
				 * logger.info("nFeeCcy2"); utilBean.setnFeeCcy2(matcher.group(1)); break; case
				 * 3 : logger.info("nFeeCcy3"); utilBean.setnFeeCcy3(matcher.group(1)); break;
				 * case 4 : logger.info("nFeeCcy4"); utilBean.setnFeeCcy4(matcher.group(1));
				 * break; case 5 : logger.info("nFeeCcy5");
				 * utilBean.setnFeeCcy5(matcher.group(1)); break; default: break; } break; case
				 * "nFeeTpCd": matcher = TAG_REGEX.matcher(thisLine); matcher.find();
				 * switch(feesize){
				 * 
				 * case 1 : utilBean.setnFeeTpCd1(matcher.group(1)); break; case 2 :
				 * utilBean.setnFeeTpCd2(matcher.group(1)); break; case 3 :
				 * utilBean.setnFeeTpCd3(matcher.group(1)); break; case 4 :
				 * utilBean.setnFeeTpCd4(matcher.group(1)); break; case 5 :
				 * utilBean.setnFeeTpCd5(matcher.group(1)); break; default: break; } break; case
				 * "nIntrchngCtg": matcher = TAG_REGEX.matcher(thisLine); matcher.find();
				 * switch(feesize){
				 * 
				 * case 1 : utilBean.setnIntrchngCtg1(matcher.group(1)); break; case 2 :
				 * utilBean.setnIntrchngCtg2(matcher.group(1)); break; case 3 :
				 * utilBean.setnIntrchngCtg3(matcher.group(1)); break; case 4 :
				 * utilBean.setnIntrchngCtg4(matcher.group(1)); break; case 5 :
				 * utilBean.setnIntrchngCtg5(matcher.group(1)); break; default: break; } break;
				 * 
				 * 
				 * case "/Fee" : feesize=feesize+1; break;
				 * 
				 * 
				 * case "nCaseNum": matcher = TAG_REGEX.matcher(thisLine); matcher.find();
				 * utilBean.setnCaseNum(matcher.group(1));
				 * 
				 * break; case "nContNum": matcher = TAG_REGEX.matcher(thisLine);
				 * matcher.find(); utilBean.setnContNum(matcher.group(1));
				 * 
				 * break; case "nFulParInd": matcher = TAG_REGEX.matcher(thisLine);
				 * matcher.find(); utilBean.setnFulParInd(matcher.group(1));
				 * 
				 * break; case "nProcCd": matcher = TAG_REGEX.matcher(thisLine); matcher.find();
				 * utilBean.setnProdCd(matcher.group(1));
				 * 
				 * break; case "nAmtBil": matcher = TAG_REGEX.matcher(thisLine); matcher.find();
				 * utilBean.setnAmtBil(matcher.group(1));
				 * 
				 * break; case "nCcyCdBil": matcher = TAG_REGEX.matcher(thisLine);
				 * matcher.find(); utilBean.setnCcyCdBil(matcher.group(1));
				 * 
				 * break; case "nConvRtBil": matcher = TAG_REGEX.matcher(thisLine);
				 * matcher.find(); utilBean.setnConvRtBil(matcher.group(1));
				 * 
				 * break; case "nMsgRsnCd": matcher = TAG_REGEX.matcher(thisLine);
				 * matcher.find(); utilBean.setnMsgRsnCd(matcher.group(1));
				 * 
				 * break; case "nRnTtlAmt": matcher = TAG_REGEX.matcher(thisLine);
				 * matcher.find(); headerUtil.setnRnTtlAmt(matcher.group(1)); break; case
				 * "nTxnCnt": matcher = TAG_REGEX.matcher(thisLine); matcher.find();
				 * headerUtil.setnTxnCnt(matcher.group(1)); transactions_count=matcher.group(1);
				 * logger.info(transactions_count); break;
				 * 
				 * case "Trl": matcher = TAG_REGEX.matcher(thisLine); matcher.find(); hdr="Trl";
				 * 
				 * break; case "/Trl": matcher = TAG_REGEX.matcher(thisLine); matcher.find();
				 * hdr="";
				 * 
				 * 
				 * break;
				 * 
				 * 
				 * case "/Txn" : feesize=1; ps.setString(1, headerUtil.getnMTI());
				 * ps.setString(2, headerUtil.getnFunCd()); ps.setString(3,
				 * headerUtil.getnRecNum()); ps.setString(4, headerUtil.getnMemInstCd());
				 * ps.setString(5, headerUtil.getnUnFlNm()); ps.setString(6,
				 * headerUtil.getnDtSet()); ps.setString(7, headerUtil.getnProdCd());
				 * ps.setString(8, headerUtil.getnSetBIN()); ps.setString(9,
				 * headerUtil.getnFlCatg()); ps.setString(10, headerUtil.getnVerNum());
				 * ps.setString(11, null); ps.setString(12, null);
				 * 
				 * ps.setString(13, headerUtil.getnTxnCnt()); ps.setString(14,
				 * headerUtil.getnRnTtlAmt()); ps.setString(15, utilBean.getnAcqInstCd());
				 * ps.setString(16, utilBean.getnAmtSet()); ps.setString(17,
				 * utilBean.getnAmtTxn()); ps.setString(18, utilBean.getnApprvlCd());
				 * ps.setString(19, utilBean.getnARD()); ps.setString(20,
				 * utilBean.getnCaseNum()); ps.setString(21, utilBean.getnCcyCdSet());
				 * ps.setString(22, utilBean.getnCcyCdTxn());
				 * 
				 * ps.setString(23, utilBean.getnConvRtSet()); ps.setString(24,
				 * utilBean.getnCrdAcpAddAdrs()); ps.setString(25, utilBean.getnCrdAcptTrmId());
				 * ps.setString(26, utilBean.getnCrdAcpZipCd()); ps.setString(27,
				 * utilBean.getnDtTmLcTxn()); ps.setString(28, utilBean.getnFunCd());
				 * ps.setString(29, utilBean.getnLtPrsntInd()); ps.setString(30,
				 * utilBean.getnMTI()); ps.setString(31, utilBean.getnPAN()); ps.setString(32,
				 * utilBean.getnRecNum()); ps.setString(33, utilBean.getnRGCSRcvdDt());
				 * 
				 * ps.setString(34, utilBean.getnSetDCInd()); ps.setString(35,
				 * utilBean.getnTxnDesInstCd()); ps.setString(36, utilBean.getnTxnOrgInstCd());
				 * ps.setString(37, utilBean.getnUID()); ps.setString(38,
				 * utilBean.getnAmtBil()); ps.setString(39, utilBean.getnCcyCdBil());
				 * ps.setString(40, utilBean.getnConvRtBil()); ps.setString(41,
				 * utilBean.getnMsgRsnCd());
				 * 
				 * ps.setString(42, utilBean.getnFeeDCInd1()); ps.setString(43,
				 * utilBean.getnFeeAmt1()); ps.setString(44, utilBean.getnFeeCcy1());
				 * ps.setString(45, utilBean.getnFeeTpCd1()); ps.setString(46,
				 * utilBean.getnIntrchngCtg1());
				 * 
				 * ps.setString(47, utilBean.getnFeeDCInd2()); ps.setString(48,
				 * utilBean.getnFeeAmt2()); ps.setString(49, utilBean.getnFeeCcy2());
				 * ps.setString(50, utilBean.getnFeeTpCd2()); ps.setString(51,
				 * utilBean.getnIntrchngCtg2()); ps.setString(52, utilBean.getnFeeDCInd3());
				 * ps.setString(53, utilBean.getnFeeAmt3()); ps.setString(54,
				 * utilBean.getnFeeCcy3()); ps.setString(55, utilBean.getnFeeTpCd3());
				 * ps.setString(56, utilBean.getnIntrchngCtg3());
				 * 
				 * ps.setString(57, utilBean.getnFeeDCInd4()); ps.setString(58,
				 * utilBean.getnFeeAmt4()); ps.setString(59, utilBean.getnFeeCcy4());
				 * ps.setString(60, utilBean.getnFeeTpCd4()); ps.setString(61,
				 * utilBean.getnIntrchngCtg4()); ps.setString(62, utilBean.getnFeeDCInd5());
				 * ps.setString(63, utilBean.getFeeAmt5()); ps.setString(64,
				 * utilBean.getnFeeCcy5()); ps.setString(65, utilBean.getnFeeTpCd5());
				 * ps.setString(66, utilBean.getnIntrchngCtg5()); ps.setString(67,
				 * headerUtil.getTrl_nFunCd()); ps.setString(68, headerUtil.getTrl_nRecNum());
				 * 
				 * ps.setString(69, "I"); ps.setString(70,filedate);
				 * 
				 * ps.addBatch();
				 * 
				 * utilBean = new RupayUtilBean();
				 ...
				 * 
				 * ps.executeBatch(); logger.info("Executed batch"); count++; }
				 * 
				 * break; default:
				 * 
				 * break; }
			...
			logger.error(" error in ReadRupay.uploadInternationalData",
					new Exception("ReadRupay.uploadInternationalData", ex));
			return false;
		}
