...
public class ReadDFSJCBMonthlyNTSLFile {
	
	private static final Logger logger = Logger.getLogger(ReadDFSJCBMonthlyNTSLFile.class);
    ...
    public HashMap<String, Object> DFSfileupload(NFSSettlementBean beanObj,MultipartFile file,Connection con) throws SQLException {
          ...
          /* String sql = "insert  into BACIDSTAT_temp (SRNO,ACCOUNTNO,Bacid,TRAN_DATE, Particulars,TRAN_RMKS ,Debit,Credit, Closing_balance,IFSC,entry_by)"
                        + " values (?,?,?,?,?,?,?,?,?,?,6346)";*/
           String sql = "INSERT INTO "+tableName+"(DATES,INTERCHANGE_FEE_TXN,CGST_TXN,SGST_TXN,IGST_TXN,INTERCHANGE_FEE,CGST_DISP,SGST_DISP,IGST_DISP,CREATEDBY,CREATEDDATE,SETTLEMENT_MONTH) "
           		+ "VALUES( ?,?,?,?,?,?,?,?,?,?,SYSDATE,?)";
...
OUTER:   for (Row r : sheet) {
                     ...
                     if (r.getRowNum() > 15) {
                     	count++;
                            int cellCount = 1;
                            for (Cell c : r) {
                            	logger.info("Row count is "+r.getRowNum());
                                   switch(formulaEvaluate.evaluateInCell(c).getCellType())
                                   {
                                 	  case Cell.CELL_TYPE_STRING:
                                 		  if(c.getStringCellValue().equalsIgnoreCase("NFS is a multilateral ATM network of member banks"))
