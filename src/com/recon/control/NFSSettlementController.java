...
public class NFSSettlementController {
	...
	@ResponseBody
	public String nfsFileUploadPost(@ModelAttribute("nfsSettlementBean") NFSSettlementBean nfsSettlementBean,
			HttpServletRequest request, @RequestParam("file") MultipartFile file, String filename, String category,
			String stSubCategory, String datepicker, HttpSession httpSession, Model model, ModelAndView modelAndView,
	...
	public ModelAndView ntslFileUploadGet(ModelAndView modelAndView, @RequestParam("category") String category,
			HttpServletRequest request) throws Exception {
	...
	@ResponseBody
	public String ntslFileUploadPost(@ModelAttribute("nfsSettlementBean") NFSSettlementBean nfsSettlementBean,
			HttpServletRequest request, @RequestParam("file") MultipartFile file, HttpSession httpSession,
			RedirectAttributes redirectAttributes) throws Exception {

		logger.info("***** NTSLFileUpload.Post Start ****");
		HashMap<String, Object> mapObj = new HashMap<String, Object>();
		logger.info("Data is " + nfsSettlementBean);
		// logger.info("Data "+filename+" "+category+" "+stSubCategory+" "+datepicker+"
		// "+cycle+" "+timePeriod);
		String Createdby = ((LoginBean) httpSession.getAttribute("loginBean")).getUser_id();
		logger.info("Created by is " + Createdby);
		nfsSettlementBean.setCreatedBy(Createdby);
		String fileName = file.getOriginalFilename();
		if (file.getOriginalFilename().contains("_") && nfsSettlementBean.getFileName().contains("NFS")) {// VALIDATING
																											// UPLOADED
																											// FILE AND
																											// CYCLE
																											// SELECTED
		...
		if (nfsSettlementBean.getTimePeriod() != null && nfsSettlementBean.getTimePeriod().equalsIgnoreCase("Daily")) {
			...
			try {
				...
				if (!fileName.contains("GL_REPORT_655")) {

					result = nfsSettlementService.validatePrevFileUpload(nfsSettlementBean);
				}
				
				if (fileName.contains("GL_REPORT_655")) {

					result = nfsSettlementService.validateClosing(fileName);
				}
	...
	public ModelAndView NFSSettlement(ModelAndView modelAndView, @RequestParam("category") String category,
			HttpServletRequest request) throws Exception {
	...
	/*
	 * @RequestMapping(value = "DownloadSettlementreport", method =
	 * RequestMethod.POST) public String
	 * NFSMonthlySettlementProcess(@ModelAttribute("nfsSettlementBean")
	 * NFSSettlementBean nfsSettlementBean,HttpServletRequest request, HttpSession
	 * httpSession,RedirectAttributes redirectAttributes,Model model) throws
	 * Exception { logger.info("***** DownloadSettlementreport.POST Start ****");
	 * //logger.info("Data "+filename+" "+category+" "+stSubCategory+" "
	 * +datepicker+" "+cycle); logger.info("NFSSettlement POST");
	 * nfsSettlementBean.setCategory("NFS_SETTLEMENT"); //
	 * nfsSettlementBean.setFileName(filename); List<Object> Excel_data = new
	 * ArrayList<Object>(); String Createdby = ((LoginBean)
	 * httpSession.getAttribute("loginBean")).getUser_id();
	 * logger.info("Created by is "+Createdby);
	 * nfsSettlementBean.setCreatedBy(Createdby);
	 * 
	 * //GETTING , IN DATE FIELD
	 * logger.info("File Name is "+nfsSettlementBean.getFileName()); boolean
	 * executed = false;
	 * 
	 ...
	 * ",", "")));
	 * 
	 * if(nfsSettlementBean.getFileName().contains("NFS")) { executed =
	 * nfsSettlementCalService.runNFSDailyProc(nfsSettlementBean); } else { executed
	 * = nfsSettlementCalService.runDFSJCBDailyProc(nfsSettlementBean); }
	 * 
	 * if(executed) { //GET DATA FOR REPORT Excel_data =
	 * nfsSettlementCalService.getDailySettlementReport(nfsSettlementBean); }
	 * model.addAttribute("ReportName",
	 * "Settlement_Cycle"+nfsSettlementBean.getCycle()); model.addAttribute("data",
	 * Excel_data); logger.
	...
	@ResponseBody
	public String NFSSettProcessValidation(@ModelAttribute("nfsSettlementBean") NFSSettlementBean nfsSettlementBean,
	...
	public String DownloadSettreport(@ModelAttribute("nfsSettlementBean") NFSSettlementBean nfsSettlementBean,
			HttpServletRequest request, HttpSession httpSession, RedirectAttributes redirectAttributes, Model model)
			throws Exception {
...
//		nfsSettlementBean.setFileName(filename);
		...
		model.addAttribute("ReportName",
				nfsSettlementBean.getFileName() + "_Settlement_Cycle" + nfsSettlementBean.getCycle());
	...
	@ResponseBody
	public String skipSettlement(String fileDate, String stSubCategory, String timePeriod, String cycle,
			String filename, @ModelAttribute("nfsSettlementBean") NFSSettlementBean nfsSettlementBean,
			HttpServletRequest request, HttpSession httpSession, RedirectAttributes redirectAttributes, Model model)
			throws Exception {
...
//METHOD FOR DAILY INCOME CALCULATION
	...
	public ModelAndView NFSInterchange(ModelAndView modelAndView, @RequestParam("category") String category,
			HttpServletRequest request) throws Exception {
	...
	@ResponseBody
	public String NFSInterchangeValidation(String fileDate, String stSubCategory, String timePeriod, String filename,
			HttpServletRequest request, HttpSession httpSession, RedirectAttributes redirectAttributes, Model model)
			throws Exception {
	...
	public String DownloadInterchangereport(@ModelAttribute("nfsSettlementBean") NFSSettlementBean nfsSettlementBean,
			HttpServletRequest request, HttpSession httpSession, RedirectAttributes redirectAttributes, Model model)
			throws Exception {
...
//		nfsSettlementBean.setFileName(filename);
		...
		if (nfsSettlementBean.getTimePeriod().equalsIgnoreCase("MONTHLY")) {
			...
			try {
				 ...
				 * SimpleDateFormat formatter = new SimpleDateFormat("dd/MM/yyyy"); String
				 * testDate = formatter.format(lastDayOfMonth);
				 * System.out.println("TEst Date "+testDate); lastDate =
				 * lastDayOfMonth.toString(); System.out.println("Last Date is "+lastDate);
...
//RECTIFYING SETTLEMENT AMOUNT
	...
	public String SettlementRectify(@ModelAttribute("nfsSettlementBean") NFSSettlementBean nfsSettlementBean,
			String category, String datepicker, String stSubCategory, String timePeriod, String cycle, String fileName,
			String rectAmt, HttpServletRequest request, HttpSession httpSession, RedirectAttributes redirectAttributes,
			Model model) throws Exception {
	...
	public ModelAndView addCooperativeBank(ModelAndView modelAndView, @RequestParam("category") String category,
			HttpServletRequest request) throws Exception {
		logger.info("***** nfsFileUpload.Get Start ****");
		NFSSettlementBean nfsSettlementBean = new NFSSettlementBean();
		logger.info("nfsFileUpload GET");
	...
	@ResponseBody
	public String addCooperativeBankPost(ModelAndView modelAndView, String bankName, String accNumber,
			HttpServletRequest request) throws Exception {
	...
	@ResponseBody
	public String saveNodalDetails(@ModelAttribute("addNewSolBean") AddNewSolBean addNewSolBean,
